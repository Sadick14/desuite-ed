...
const form = useForm({
  ...
  level: props.class?.level || '',
  selected_courses: props.selectedCourseIds ? [...props.selectedCourseIds] : [],
});
...
function submit() {
  // Get class ID from URL since props might not include it
  const url = window.location.pathname;
  const classIdMatch = url.match(/\/classes\/(\d+)\/edit/);
  const classId = classIdMatch ? classIdMatch[1] : props.class?.id;
...
<template>
  <Head :title="`Edit ${props.class.name}`" />
  ...
  <div class="min-h-screen bg-gradient-to-b from-blue-50 via-white to-blue-50">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-3xl font-bold text-gray-900">Edit Class: {{ form.name || props.class?.name || 'Loading...' }}</h1>
      </div>
        ...
        <div class="bg-white rounded-2xl border border-blue-100 shadow-sm overflow-hidden space-y-6 p-6">
          ...
          <div>
            <div class="flex items-center justify-between mb-3">
              <label class="block text-sm font-semibold text-gray-900">Class Courses</label>
            ...
            <div class="flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <p class="text-sm text-blue-900">
                ...
                <span v-if="selectedCount === 1">course assigned</span>
                <span v-else>courses assigned</span>
              </p>
            </div>
